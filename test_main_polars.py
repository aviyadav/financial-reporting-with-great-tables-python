...
def render(name, gt):
    html = gt.as_raw_html()
    print(f"  {name}: OK ({len(html)} bytes)")


def test_basic_financial_statement():
    gt = (
        GT(income_stmt)
        .tab_header(
        ...
        .fmt_currency(columns=["Q3 2025", "Q4 2025"], currency="USD", decimals=0)
        .fmt_percent(columns=["YoY Change"], decimals=1)
        .opt_stylize(style=1, color="gray")
    )
    render("basic_financial_statement", gt)


def test_accounting_notation():
    gt = (
        GT(income_stmt)
    ...
    render("accounting_notation", gt)


def test_smart_rows_grand_total():
    gt = (
        ...
        .fmt_percent(columns=["YoY Change"], decimals=1)
        .grand_summary_rows(
            fns={"Total": pl.col("Q3 2025", "Q4 2025").sum()},
            fmt=vals.fmt_currency,
        )
    )
    render("smart_rows_grand_total", gt)
...
def test_multiple_currencies():
    ...
    render("multiple_currencies", gt)


def test_column_spanners_hierarchical_headers():
    ...
    render("column_spanners_hierarchical_headers", gt)


def test_portfolio_performance_dashboard():
    ...
    render("portfolio_performance_dashboard", gt)


if __name__ == "__main__":
    print("Running all render tests...")
    test_basic_financial_statement()
    test_accounting_notation()
    test_smart_rows_grand_total()
    test_multiple_currencies()
    test_column_spanners_hierarchical_headers()
    test_portfolio_performance_dashboard()
