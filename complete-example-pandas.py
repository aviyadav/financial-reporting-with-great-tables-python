...
# gapminder is already a pandas DataFrame — no conversion needed
gapminder_df = gapminder

# Use a fixed seed for reproducibility so the same 6 countries are selected every run
random_countries = pd.Series(gapminder_df["country"].unique()).sample(
    n=6, random_state=42
)

# All data wrangling done in pandas
gapminder_summary = (
    gapminder_df[gapminder_df["country"].isin(random_countries)]
    .sort_values("year")  # pre-sort so list aggs respect year order
    .groupby("country", as_index=False)
    .agg(
        years=("year", list),
        pop_trend=("pop", list),
        population_2007=("pop", "last"),
    )
    .assign(year=lambda df: df["years"].str[-1])[
        ["country", "year", "population_2007", "pop_trend"]
    ]
    .sort_values("population_2007", ascending=False)
    .reset_index(drop=True)
)

# Convert to Polars before passing to GT to avoid a great_tables bug where
# pd.isna() called on a list column raises "truth value of an array is ambiguous"
...
output_path = "output-pandas.html"
