...
def basic_financial_statement(income_stmt):
...
# Accounting Notation
def accounting_notation(income_stmt):
    financial_table = (
        GT(income_stmt)
...
# Summary Rows with Grand Totals
def smart_rows_grand_total(income_stmt):
    financial_table = (
        GT(income_stmt, rowname_col="Line Item")
        .tab_header(title="Income Statement")
        .fmt_currency(columns=["Q3 2025", "Q4 2025"], currency="USD", decimals=0, accounting=True)
        .fmt_percent(columns=["YoY Change"], decimals=1)
        .grand_summary_rows(
            fns={"Total": lambda df: df[["Q3 2025", "Q4 2025"]].sum()}
        )
    )
...
# Column Spanners for Hierarchical Headers
...
def portfolio_performance_dashboard():
    ...
    portfolio_table = (
        ...
        .tab_source_note(
            "Data is delayed by 15 minutes. Portfolio value: $533,017.50"
        )
        .opt_stylize(style=6, color='blue')
    )

    portfolio_table.show()
    # html_output = portfolio_table.as_raw_html()
    # print(html_output)
    # latex_output = portfolio_table.as_latex()
    # print(latex_output)
    # portfolio_table.save("portfolio_table.png")


if __name__ == "__main__":

    # Sample income statement data using pandas
    income_stmt = pd.DataFrame({
        ...
        'Q3 2025': [2_450_000, -920_000, 1_530_000, -680_000, 850_000, -45_000, 805_000],
        'Q4 2025': [2_890_000, -1_050_000, 1_840_000, -720_000, 1_120_000, -42_000, 1_078_000],
